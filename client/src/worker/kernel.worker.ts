...
function emitTick() {
    const cpuUsage = clamp(processes.reduce((sum, process) => sum + process.cpu, 0), 0, 100)
    const memUsageMb = processes.reduce((sum, process) => sum + process.mem, 0)
    const memUsage = clamp((memUsageMb / TOTAL_MEMORY_MB) * 100, 0, 100)
    const diskUsage = clamp(processes.reduce((sum, process) => sum + process.disk, 0), 0, 100)
    const netUsage = clamp(processes.reduce((sum, process) => sum + process.net, 0), 0, 100)

    const targetLatency = 16 + netUsage * 0.9 + (Math.random() - 0.5) * 12
